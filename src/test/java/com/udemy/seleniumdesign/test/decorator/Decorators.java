...
public class Decorators {
    ...
    private static final Consumer<DashboardPage> suComponentPresent = (dp) -> shouldDisplay(
            dp.getSuperUserComponents());
    // private static final Consumer<DashboardPage> suComponentNotPresent = (dp) ->
    // shouldNotDisplay(
    // dp.getSuperUserComponents());
    private static final Consumer<DashboardPage> guestComponentPresent = (dp) -> shouldDisplay(dp.getGuestComponents());
    // private static final Consumer<DashboardPage> guestComponentNotPresent = (dp)
    // -> shouldNotDisplay(
    ...
    public static final Consumer<DashboardPage> adminPage = adminSelection.andThen(guestComponentPresent)
            .andThen(suComponentPresent).andThen(adminComponentPresent);
